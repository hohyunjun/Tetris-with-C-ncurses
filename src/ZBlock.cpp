...
bool ZBlock::insertBlock(int &t_x, int &t_y, Cell **cells){

           t_y = 3; t_x = 4; // 현재 맨 아래 왼쪽
	   rotate = 0;

           if(cells[t_y][t_x] == VOID){
                   cells[t_y][t_x] = Z; // 한칸 위
                   cells[t_y-1][t_x] = Z; // 두칸 위
                   cells[t_y-1][t_x+1] = Z; // 세칸 위
                   cells[t_y-2][t_x+1] = Z; // 네칸 위
                   return true;
           }
           else return false;
}

bool ZBlock::moveBlockLeft(int &t_x, int &t_y, Cell **cells){
	...
	// 현재 위치는 맨 아래 왼쪽
        if(cells[t_y][t_x-1] == VOID && cells[t_y-1][t_x-2] == VOID){
			if(t_x>1){
				...
				cells[t_y][t_x-1] = Z; // 원래 있던값들 지워준다
				cells[t_y][t_x] = Z;
				cells[t_y-1][t_x-1] = Z;
				cells[t_y-1][t_x-2] = Z;
				t_x = t_x-1;
...
bool ZBlock::moveBlockRight(int &t_x, int &t_y, Cell **cells){

	if(rotate == 0 || rotate == 2){
	// 현재 위치는 맨 아래 왼쪽
        if(cells[t_y-2][t_x+2] == VOID && cells[t_y-1][t_x+2] == VOID){
...
bool ZBlock::moveBlockDown(int &t_x, int &t_y, Cell **cells){
	...
	// 현재 위치는 맨 아래 왼쪽
        ...
        if(cells[t_y+1][t_x] == VOID && cells[t_y+1][t_x+1] == VOID){
			...
			cells[t_y+1][t_x] = Z; // 원래 있던값들 지워준다
			cells[t_y+1][t_x+1] = Z;
			cells[t_y][t_x] = Z;
			cells[t_y][t_x-1] = Z;
                        t_y = t_y+1; // 현재 tx ty는 맨 아래
...
void ZBlock::dropBlock(int &t_x, int &t_y, Cell **cells){
...
bool ZBlock::rotateBlock(int &t_x, int &t_y, Cell **cells){
